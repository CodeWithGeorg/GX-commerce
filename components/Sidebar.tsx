...
const Sidebar: React.FC<SidebarProps> = ({ onToggleCart, onToggleChat, activeSection, setActiveSection }) => {
  const NavItem = ({ icon: Icon, label, id, onClick }: { icon: any, label: string, id?: string, onClick?: () => void }) => (
    ...
    >
      ...
      <div className="hidden md:block absolute left-14 px-2 py-1 theme-bg-tertiary border theme-border rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-50 shadow-lg">
        {label}
      </div>
    </button>
  );
  ...
  return (
    <aside className="fixed bottom-0 md:bottom-auto left-0 md:top-0 h-16 md:h-screen w-full md:w-14 theme-bg-primary border-t md:border-t-0 md:border-r theme-border flex flex-row md:flex-col items-center px-2 md:px-0 md:py-6 z-50">
      ...
      <div className="flex flex-row md:flex-col gap-1 md:gap-4 flex-1 w-full h-full items-center">
        ...
        {/* 
          Dynamic Spacer: 
          Uses flex-1 only on md screens (desktop) to occupy all available space in the column.
          This pushes subsequent items to the bottom of the sidebar.
        */}
