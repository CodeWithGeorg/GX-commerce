      ...
      case 'search':
        return (
          <section className="container mx-auto px-8 py-12 animate-in fade-in duration-500">
            <div className="mb-12">
              <h2 className="text-4xl font-orbitron font-black mb-6 flex items-center gap-4 theme-text-primary">
              ...
              <div className="relative group max-w-2xl">
                ...
                <input 
                  ...
                  className="w-full theme-bg-secondary border theme-border focus:border-[#fa1e4e] outline-none theme-text-primary font-orbitron py-4 pl-12 pr-6 rounded-sm transition-all text-xl"
                />
              </div>
            </div>
        ...
        return (
          <section className="container mx-auto px-8 py-12 animate-in fade-in duration-500">
            <div className="mb-12">
              ...
              <p className="theme-text-secondary font-orbitron text-xs tracking-widest uppercase">Your curated high-performance list</p>
            </div>
            ...
            {filteredProducts.length > 0 ? (
              ...
              <div className="py-20 text-center border border-dashed theme-border rounded-lg">
                <p className="theme-text-secondary font-orbitron text-lg">Wishlist protocol empty. Mark targets to save them here.</p>
          ...
          <>
            <section className="relative h-[400px] overflow-hidden">
              ...
              <div className="relative h-full container mx-auto px-8 flex flex-col justify-center">
                ...
                <h1 className="text-6xl md:text-8xl font-orbitron font-black leading-none mb-6 theme-text-primary">
                  BEYOND <br />
                  <span className="text-[#fa1e4e] gx-glow">LIMITS.</span>
                </h1>
                <p className="max-w-xl theme-text-secondary text-lg mb-8">
                ...
                <div className="flex gap-4">
                  <button onClick={() => document.getElementById('market-grid')?.scrollIntoView({ behavior: 'smooth' })} className="bg-[#fa1e4e] hover:bg-[#ff4e7e] text-white font-orbitron font-bold py-3 px-8 rounded-sm transition-all shadow-[0_0_20px_rgba(250,30,78,0.3)]">BROWSE GEAR</button>
                  <button onClick={() => setActiveSection('trailers')} className="border theme-border hover:border-[#fa1e4e] theme-text-primary font-orbitron font-bold py-3 px-8 rounded-sm transition-all">TRAILER</button>
                </div>
              </div>
            </section>

            <div id="market-grid" className="sticky top-0 z-40 theme-bg-primary/90 backdrop-blur-md border-b theme-border py-4 px-8">
              <div className="flex items-center gap-6 overflow-x-auto no-scrollbar">
                {CATEGORIES.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)} className={`text-xs font-orbitron uppercase tracking-widest whitespace-nowrap px-4 py-2 border border-transparent transition-all ${activeCategory === cat ? 'text-[#fa1e4e] border-b-[#fa1e4e]' : 'theme-text-secondary hover:theme-text-primary'}`}>{cat}</button>
                ))}
              </div>
            </div>
            ...
            <section className="container mx-auto px-8 py-12">
              <div className="flex justify-between items-end mb-10">
                <div>
                  <h2 className="text-3xl font-orbitron font-bold mb-2 theme-text-primary">HOT <span className="text-[#fa1e4e]">DROPS</span></h2>
                  <p className="theme-text-secondary">Live inventory of high-performance components</p>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredProducts.map(p => (
  ...
  return (
    <div className="min-h-screen theme-bg-primary theme-text-primary transition-colors duration-300">
      ...
      <main className="pl-14">
        ...
        <section className="theme-bg-secondary py-20 border-t theme-border">
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-[#fa1e4e]/10 border border-[#fa1e4e]/30 rounded-full flex items-center justify-center mb-6">
                  <f.icon className="text-[#fa1e4e]" size={32} />
                </div>
                <h3 className="text-xl font-orbitron font-bold mb-4 theme-text-primary">{f.title}</h3>
                <p className="theme-text-secondary text-sm leading-relaxed">{f.text}</p>
              </div>
            ))}
          </div>
        </section>
